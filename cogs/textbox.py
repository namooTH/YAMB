...
class textbox(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.channel.id == 1234430787924000778:
            image = await self.generatetextbox(avatar=Image.open(BytesIO(requests.get(message.author.avatar.url).content)), text=message.clean_content, name=None, animated=None)
        ...
        if message.content.lower() == "mtbq":
            ...
            if messager.author == self.bot.user:
                return
            image = await self.generatetextbox(avatar=Image.open(BytesIO(requests.get(messager.author.avatar.url).content)), text=messager.content, name=messager.author.name, animated=None)
